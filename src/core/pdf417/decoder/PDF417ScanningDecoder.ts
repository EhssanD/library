...
export default /*public final*/ class PDF417ScanningDecoder {
  ...
  public static decode(
    ...
    for (
      let barcodeColumnCount /*int*/ = 1;
      barcodeColumnCount <= maxBarcodeColumn;
      barcodeColumnCount++
    ) {
      let barcodeColumn: int = leftToRight
        ? barcodeColumnCount
      ...
      for (
        let imageRow /*int*/ = boundingBox.getMinY();
        ...
        let codeword: Codeword = PDF417ScanningDecoder.detectCodeword(
          image,
          boundingBox.getMinX(),
          boundingBox.getMaxX(),
          leftToRight,
  ...
  private static merge(
    ...
    let barcodeMetadata: BarcodeMetadata = PDF417ScanningDecoder.getBarcodeMetadata(
      leftRowIndicatorColumn,
      rightRowIndicatorColumn
    );
    ...
    // throw "Found";
    let boundingBox: BoundingBox = BoundingBox.merge(
  ...
  private static adjustBoundingBox(
    ...
    for (
      let row /*int*/ = codewords.length - 1;
  ...
  private static getRowIndicatorColumn(
    ...
    let rowIndicatorColumn: DetectionResultRowIndicatorColumn = new DetectionResultRowIndicatorColumn(
      boundingBox,
      leftToRight
    );
    for (let i /*int*/ = 0; i < 2; i++) {
      ...
      for (
        let imageRow /*int*/ = Math.trunc(<int>Math.trunc(startPoint.getY()));
        imageRow <= boundingBox.getMaxY() && imageRow >= boundingBox.getMinY();
        imageRow += increment
      ) {
        let codeword: Codeword = PDF417ScanningDecoder.detectCodeword(
          image,
  ...
  private static adjustCodewordCount(
    ...
    let calculatedNumberOfCodewords: int =
  ...
  private static createDecoderResult(
    ...
    let barcodeMatrix: BarcodeValue[][] = PDF417ScanningDecoder.createBarcodeMatrix(
      detectionResult
    );
    PDF417ScanningDecoder.adjustCodewordCount(detectionResult, barcodeMatrix);
    let erasures /*Collection<Integer>*/ = new Array<number>();
    let codewords: Int32Array = new Int32Array(
      detectionResult.getBarcodeRowCount() *
        detectionResult.getBarcodeColumnCount()
    );
    ...
    let ambiguousIndexValues: Int32Array[] = new Array<Int32Array>(
  ...
  private static createDecoderResultFromAmbiguousValues(
    ...
    let ambiguousIndexCount: Int32Array = new Int32Array(
  ...
  private static createBarcodeMatrix(
    ...
    let barcodeMatrix: BarcodeValue[][] = Array.from(
  ...
  private static getStartColumn(
    ...
    while (
      ...
      barcodeColumn -= offset;
      for (let previousRowCodeword /*Codeword*/ of detectionResult
        .getDetectionResultColumn(barcodeColumn)
        .getCodewords()) {
  ...
  private static detectCodeword(
    ...
    let moduleBitCount: Int32Array = PDF417ScanningDecoder.getModuleBitCount(
      image,
      minColumn,
      maxColumn,
      leftToRight,
    ...
    let decodedValue: int = PDF417CodewordDecoder.getDecodedValue(
      moduleBitCount
    );
  ...
  private static getModuleBitCount(
    ...
    while (
      (leftToRight ? imageColumn < maxColumn : imageColumn >= minColumn) &&
  ...
  private static decodeCodewords(
    ...
    let correctedErrorsCount: int = PDF417ScanningDecoder.correctErrors(
      codewords,
      erasures,
      numECCodewords
    );
    ...
    let decoderResult: DecoderResult = DecodedBitStreamParser.decode(
      codewords,
      '' + ecLevel
    );
